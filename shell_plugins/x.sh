...
x() {
  ...
  ai | airbox | ap | bc | cal | cat-weight | cert | county-sunrise | data | editorconfig | eq | erl | ex | focus | fx | getnf | gif | hbt | html | ico | jq | kibble | mh | paisa | pdf | png | py | rm | sh | smolsite | sn | sqlite | ssh-keygen | ss | st | touch | tree | tz | uri | venv | w8 | webp | whois | zip)
    cmd="$1" && shift
