...
def around_const_let_var(snip):
    return (
        re.search("const|let|var", snip.buffer[snip.line])
        and "=" not in snip.buffer[snip.line]
    )
...
def around_first_line_or_require(snip):
    return "const" not in snip.buffer[snip.line] and (
        snip.line == 0 or "require" in snip.buffer[snip.line - 1]
    )
...
def around_switch(snip):
    return re.search("break|switch", snip.buffer[snip.line - 1]) or (
        snip.line - 2 >= 0
        and "case" in snip.buffer[snip.line - 2]
        and "return" in snip.buffer[snip.line - 1]
    )
